...
class ApiServer : AbstractVerticle() {
    ...
    private fun createRoutes(dbClient : JDBCClient, eventBus : EventBus) = Router.router(vertx).apply {
        ...
        // Setup APIs
        get("/api/provinces").handler(handleGetProvinces)
        get("/api/base_lmr_history").handler(handleHistoricalRatings)
        get("/api/base_pro_forma_lmr_history").handler(handleProFormaRatings)
        get("/api/base_lmr_forecast").handler(handleBaseForecast)
        get("/api/combined_lmr_trend").handler(handleFullInclusionTrend)
        get("/api/report_dates").handler(handleReportDates)
        get("/api/latest_report_date").handler(handleLatestReportDate)
        get("/api/historical_netbacks").handler(handleHistoricalNetbacks)
        get("/api/lmr_details").handler(handleLiabilityDetails)
        post("/api/upload_ab_liabilities").handler(handleAbLiabilityUpload)
        post("/api/upload_hierarchy_mapping").handler(handleHierarchyMappingUpload)
        get("/api/export_historical_report").handler(handleExportLiabilities)
        post("/api/export_historical_report").handler(handleExportLiabilities)
        get("/api/dispositions").handler(handleGetDispositions)
        post("/api/create_disposition").handler(handleCreateDisposition)
        post("/api/toggle_disposition").handler(handleToggleDisposition)
        post("/api/delete_disposition").handler(handleDeleteDisposition)
        get("/api/acquisitions").handler(handleGetAcquisitions)
        post("/api/create_acquisition").handler(handleCreateAcquisition)
        post("/api/delete_acquisition").handler(handleDeleteAcquisition)
        post("/api/toggle_acquisition").handler(handleToggleAcquisition)
        get("/api/acquisition_lmr_history").handler(handleAcquisitionRatings)
        get("/api/acquisition_lmr_forecast").handler(handleForecastAcquisition)
        get("/api/acquisition_lmr_trend").handler(handleFullAcquisitionsTrend)
        get("/api/aro_plans").handler(handleGetAroPlans)
        post("/api/create_aro_plan").handler(handleCreateAroPlan)
        post("/api/delete_aro_plan").handler(handleDeleteAroPlan)
        post("/api/toggle_aro_plan").handler(handleToggleAroPlan)
        get("/api/growth_entities").handler(handleGrowthEntities)
        get("/api/growth_forecasts").handler(handleGrowthForecast)
